...
GENESIS_FILE="$HOME"/.terra/config/genesis.json
if [ -f "$GENESIS_FILE" ]; then
  echo "$GENESIS_FILE exists..."
else
  echo "$GENESIS_FILE does not exist. Generating..."

  terrad init --chain-id "$CHAIN_ID" "$MONIKER"
  ...
  sed -i 's/keyring-backend = "os"/keyring-backend = "test"/' "$HOME"/.terra/config/client.toml
fi

APP_TOML_CONFIG="$HOME"/.terra/config/app.toml
APP_TOML_CONFIG_NEW="$HOME"/.terra/config/app_new.toml
CONFIG_TOML_CONFIG="$HOME"/.terra/config/config.toml
...
if ! terrad keys show validator $KEYRING; then
  (echo "$PASSWORD"; echo "$PASSWORD") | terrad keys add validator $KEYRING

  # hardcode the validator account for this instance
  echo "$PASSWORD" | terrad add-genesis-account validator "1000000000$STAKE,1000000000$FEE" $KEYRING

  # (optionally) add a few more genesis accounts
  for addr in "$@"; do
    echo $addr
    terrad add-genesis-account "$addr" "1000000000$STAKE,1000000000$FEE"
  done

  # submit a genesis validator tx
  ## Workraround for https://github.com/cosmos/cosmos-sdk/issues/8251
  (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | terrad gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID" --amount="250000000$STAKE" $KEYRING
  ## should be:
